...
namespace QLBH_NHOM.Controllers
{
    public class AccountController : Controller
    {
        ...
        public int CheckUserNV(string username, string pws)
        {
            ...
            if (kq.Count() > 0)
            {
                ...
                return (int) kq.FirstOrDefault().QUYENNV;
            }
            else
            {
        ...
        public ActionResult DangNhap(NHANVIEN qt)
        {
            if (ModelState.IsValid)
            {
                if (CheckUserNV(qt.TENDANGNHAP, qt.MATKHAU)==1)
                {
                    FormsAuthentication.SetAuthCookie(qt.TENDANGNHAP, true);
                    return RedirectToAction("Intro","Admin/HOMEs");
                   
                }
                else if (CheckUserNV(qt.TENDANGNHAP, qt.MATKHAU) == 3)
                {
                    FormsAuthentication.SetAuthCookie(qt.TENDANGNHAP, true);
                    return RedirectToAction("DonCuaToi", "Admin/DONHANGs");
                }    
                else
