...
namespace QLBH_NHOM.Areas.Admin.Controllers
{
    public class DONHANGsController : Controller
    {
        ...
        public ActionResult DonHangShipper()
        {
            var dONHANGs = db.DONHANGs.Include(d => d.KHACHHANG).Include(d => d.NHANVIEN).Include(d => d.NHANVIEN1).Include(d => d.TINHTRANGDON);
            return View(dONHANGs.ToList());
        }
        public ActionResult DonCuaToi()
        {
            string manv = (String)Session["MaNV"];
            var myInvorce = (from dh in db.DONHANGs
                       where dh.MANV_GIAO == manv
                             select dh);
            return View(myInvorce.ToList());
        }
